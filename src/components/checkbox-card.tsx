...
export function CheckboxCard({ id, site }: { id: number; site?: Site }) {
  ...
  const copyScreen = async () => {
    try {
      if (!site?.screenUrl) {
        toast.error("No screen image available to copy")
        return
      }

      // Try to copy the image URL as text (most reliable fallback)
      ...
      toast.error("Failed to copy screen. Please try again.")
    }
  }
      ...
      <TooltipProvider delayDuration={0}>
        <div className="group relative flex flex-col gap-y-3 md:gap-y-4">
          <div className="relative rounded-[28px] overflow-hidden w-full">
            <div className="rounded-[28px] overflow-hidden w-full h-auto max-h-[680px] bg-muted flex items-center justify-center aspect-[9/16]">
              {site?.screenUrl ? (
                <Image
                  src={site.screenUrl}
