...
export function TreeView({
  ...
  // Update expanded IDs when defaultExpandedIds changes
  React.useEffect(() => {
    ...
    (nodeId: string) => {
      setExpandedIds((prev) => {
        ...
        if (isExpanded) {
          ...
          // Open this node and close others at the same level
          newSet.add(nodeId);
          
          // Find and close siblings at the same level
          const findSiblings = (nodes: TreeNode[], targetId: string, level = 0): string[] => {
            const siblings: string[] = [];
            for (const node of nodes) {
              if (level === 0) {
                // At root level, close other root nodes
                if (node.id !== targetId) {
                  newSet.delete(node.id);
                }
              } else if (node.children) {
                const childSiblings = findSiblings(node.children, targetId, level - 1);
                siblings.push(...childSiblings);
              }
            }
            return siblings;
          };
          
          findSiblings(data, nodeId);
        }
